...
func CancelHandler(c *gin.Context) {
	...
	instance, err := common.Discovery.Registry.Cancel(req.Env, req.AppId, req.Hostname, req.LatestTimestamp)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"code":    err.Code(),
			"message": err.Error(),
	...
	if !req.Replication {
		common.Discovery.Nodes.Load().(*registry.Nodes).Replicate(global.Cancel, instance)
	}
	c.JSON(http.StatusOK, gin.H{
